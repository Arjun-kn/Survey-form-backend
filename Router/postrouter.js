...
postapp.get("/userpost", authenticateUser, (req, res) => {
  jwt.verify(userId, process.env.Skey, (err, auth) => {
    if (err) {
      res.status(404).json({ message: "Invalid token" });
    } else {
      Postdata.find({ user: auth.id })
        .then((record) => {
...
postapp.post("userpost",authenticateUser,(req,res)=>{
    jwt.verify(userId,process.env.Skey,(err,auth)=>{
        if(err){
          ...
          newdata.save().then(record=>{
            res.status(200).json({message:"Data post successfully", data:record})
          }).catch(err=>{
